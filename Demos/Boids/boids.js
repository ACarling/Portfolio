...
class BoidController {
    ...
    constructor() {
        ...
        let tank = new THREE.BoxGeometry(20*this.width+this.margin,20*this.depth+this.margin, this.height+this.margin); 

        tankMaterial = new THREE.MeshLambertMaterial( { color: palletGround } );

        let tankGeom = new THREE.Mesh( tank, tankMaterial ); 

        scene.add( tankGeom ); 
        tankGeom.receiveShadow = true; 
        tankGeom.lookAt(0,1,0);
        tankGeom.position.y -= (this.height+this.margin);
...
// chuck noise(boid.x,boid..y) on this homing factor
        ...
        boid.dy += (0 - boid.y) * this.homingFactor * dst * noiseVal;
